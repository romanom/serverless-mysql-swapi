...
export const run = async (event, context, callback) => {
    ...
    peopleController.init(headers, params, requestContext);
    ...
    async function callbackFilter() {
        ...
        if(requestIsOk) {
            ...
            switch (key) {
                case '/api/people/{id} GET':
                    
                    response = await peopleController.getById(params.id);
                    break;
                case '/api/people GET':
                    
                    response = await peopleController.get();
                    break;
                case '/api/people POST':
                    response = await peopleController.create(JSON.parse(event.body));
                default:
